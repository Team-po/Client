...
const summaryCards = [
	...
	},
	{
		...
		value: "3 Items",
		description:
			"요청 사용자 간 매칭 로직과 결과 생성, 팀 생성 연결 흐름을 구현하고 있습니다.",
		icon: Clock3,
		tone: "accent",
	},
	{
		...
		value: "3 Items",
		description:
			"팀 생성 안정화, 인증부터 팀 생성까지의 전체 흐름 통합, 서버 배포를 진행합니다.",
		icon: Rocket,
		tone: "chart2",
	},
] as const;
...
const inProgressItems = [
	{
		title: "매칭 요청된 사용자들 간 매칭 로직 구현",
		description:
			"매칭 요청 집합 안에서 실제로 어떤 사용자가 어떤 팀 후보로 연결될지 결정하는 핵심 로직을 설계하고 있습니다.",
		icon: GitBranch,
		keywords: ["Matching Logic", "Selection", "Core Engine"],
	},
	{
		title: "매칭 결과 생성 및 연결 흐름 구현",
		description:
			"매칭 결과를 데이터로 생성하고 후속 처리 단계로 전달하는 흐름을 구현해 실제 동작 가능한 파이프라인을 만들고 있습니다.",
		icon: Layers3,
		keywords: ["Result Generation", "Pipeline", "Connection"],
	},
	{
		title: "매칭 완료 후 팀 생성까지 이어지는 로직 구현",
		description:
			"매칭이 끝난 뒤 결과를 팀 도메인으로 넘겨 자동으로 팀이 생성되도록 연결 흐름을 완성하는 단계입니다.",
		icon: FolderKanban,
		keywords: ["Team Create", "Post Matching", "Flow Link"],
	},
] as const;

const plannedItems = [
	{
		title: "매칭 완료 후 팀 생성 흐름 안정화",
		description:
			"현재 구현 중인 팀 생성 연결 흐름의 예외 상황과 상태 전이를 정리해 안정적으로 동작하도록 다듬습니다.",
		icon: UsersRound,
		keywords: ["Stabilize", "Team Flow", "Edge Cases"],
	},
	{
		title: "인증 → 매칭 요청 → 매칭 완료 → 팀 생성 흐름 통합",
		description:
			"사용자 입장에서 핵심 가치가 드러나는 end-to-end 시나리오를 하나의 연결된 흐름으로 통합합니다.",
		icon: Layers3,
		keywords: ["End-to-End", "Integration", "User Journey"],
	},
	{
		title: "서버 배포",
		description:
			"주요 흐름이 연결되는 시점에 맞춰 서버를 배포하고 실제 환경에서 서비스 동작을 점검할 예정입니다.",
		icon: Rocket,
		keywords: ["Deployment", "Server", "Release"],
	},
] as const;
...
export function TeamPoPresentationSixthView() {
	...
	return (
		...
		>
			...
			<main className="relative pb-16">
					...
					<Container className="presentation-slide-inner relative rounded-[2rem] border border-border/60 bg-white/72 px-7 py-9 shadow-panel backdrop-blur-sm md:px-10 md:py-11">
						<div className="flex h-full flex-col gap-10">
							<div className="flex flex-wrap items-center gap-4">
								...
								<Badge variant="neutral">2026. 4. 6.</Badge>
								...
								<div className="relative flex h-full w-full flex-col justify-between gap-10">
									<div className="space-y-4">
										...
										<p className="max-w-3xl text-lg leading-8 text-muted-foreground md:text-[1.3rem]">
											비밀번호 수정, 회원탈퇴, 매칭 요청/취소/상태 조회 API 구현을
											완료했고, 이번 주에는 매칭 로직과 팀 생성 연결 흐름을
											구현하고 있습니다.
										</p>
									</div>
						...
						<div className="flex h-full flex-col rounded-[2rem] border border-emerald-300/40 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(255,255,255,0.98))] p-[1.625rem] shadow-panel">
							<div>
								...
								<p className="mt-[1.125rem] max-w-2xl text-base leading-8 text-emerald-950/80">
									계정 관리 기능을 확장했고, 매칭 요청부터 취소와 상태 조회까지
									핵심 API를 정리했습니다.
								</p>
							</div>
				...
				<PresentationSlide
					description="현재 구현은 요청된 사용자들을 실제 매칭 결과와 팀 생성 흐름으로 연결하는 핵심 로직에 집중되어 있습니다."
					id="in-progress"
					label="02"
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-4 text-base leading-7 text-amber-950/80">
										지금은 매칭 결과를 만들고, 그 결과가 팀 생성까지 이어지도록
										핵심 로직을 연결하는 단계입니다.
									</p>
								</div>
								...
								<div className="grid gap-3">
									{[
										"매칭 요청된 사용자들 간 매칭 로직 구현",
										"매칭 결과 생성 및 연결 흐름 구현",
										"매칭 완료 후 팀 생성까지 이어지는 로직 구현",
									].map((item, index) => (
										<div
				...
				<PresentationSlide
					description="다음 단계는 팀 생성 연결 흐름을 안정화하고, 인증부터 팀 생성까지의 전체 여정을 하나의 제품 흐름으로 묶는 것입니다."
					id="planned"
					label="03"
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-2 text-base leading-6 text-sky-950/80">
										팀 생성 안정화, end-to-end 통합, 그리고 서버 배포까지
										이어갑니다.
									</p>
								</div>
