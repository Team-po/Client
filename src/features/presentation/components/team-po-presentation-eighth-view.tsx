...
const summaryCards = [
	{
		...
		value: "5 Groups",
		description:
			"계정, 매칭, 팀스페이스, 인증·배포, 스토리지와 코드 품질을 정리했습니다.",
		icon: BadgeCheck,
		tone: "primary",
	},
	{
		...
		value: "1 Item",
		description:
			"매칭 요청 접수부터 수락·거절 처리까지 핵심 로직을 다듬고 있습니다.",
		icon: Clock3,
		tone: "accent",
	},
	{
...
const completedItems = [
	{
		title: "계정·매칭 기반 기능 안정화",
		description:
			"비밀번호 수정, 회원탈퇴, 매칭 요청·취소·상태 조회 API를 구현해 기본 사용 흐름을 정리했습니다.",
		icon: KeyRound,
	},
	{
		title: "팀스페이스 생성 흐름 구축",
		description:
			"매칭 이후 생성된 팀이 협업 공간을 바로 사용할 수 있도록 팀스페이스 생성 API와 초기 연결 흐름을 구현했습니다.",
		icon: FolderKanban,
	},
	{
		title: "이메일 인증 및 배포 환경 정리",
		description:
			"이메일 인증 서비스를 구현하고 Terraform, GitHub Actions, 443 포트, CORS 설정을 정리했습니다.",
		icon: BadgeCheck,
	},
	{
		title: "이미지 스토리지 개선",
		description:
			"이미지 저장 방식을 S3 기반으로 전환해 파일 저장 구조를 더 명확하게 개선했습니다.",
		icon: Layers3,
	},
	{
		title: "코드 품질 정리",
		description:
			"예외 처리 방식 통일, 코드 포매터 적용, LoginUser 어노테이션 수정, 환경변수 예시 파일 정리를 진행했습니다.",
		icon: Target,
	},
] as const;
...
const inProgressItems = [
	{
		title: "매칭 시스템 구현",
		description:
			"매칭 요청 접수부터 상대방 확인, 수락·거절 처리까지 매칭 전반의 핵심 로직을 구현하고 코드 리뷰 피드백을 반영하고 있습니다.",
		icon: GitBranch,
		keywords: ["Matching Flow", "Accept / Reject", "Review"],
	},
] as const;
...
const plannedItems = [
	{
		title: "팀스페이스 내 세부사항",
		description:
			"팀스페이스에서 다룰 협업 규칙, 체크리스트, 운영 정보 등 세부 기능 범위를 협의하고 스펙 확정 후 구현에 착수할 예정입니다.",
		icon: Layers3,
...
export function TeamPoPresentationEighthView() {
	...
	return (
		...
		>
			...
			<main className="relative pb-16">
					...
					<Container className="presentation-slide-inner relative rounded-[2rem] border border-border/60 bg-white/72 px-7 py-9 shadow-panel backdrop-blur-sm md:px-10 md:py-11">
						<div className="flex h-full flex-col gap-10">
							<div className="flex flex-wrap items-center gap-4">
								...
								<Badge variant="neutral">2026. 4. 27.</Badge>
								...
								<div className="relative flex h-full w-full flex-col justify-between gap-10">
									<div className="space-y-4">
										...
										<p className="max-w-3xl text-lg leading-8 text-muted-foreground md:text-[1.3rem]">
											계정과 매칭 기반 기능, 팀스페이스 생성, 이메일 인증, 배포
											환경, 이미지 스토리지와 코드 품질을 정리했습니다. 현재는
											매칭 시스템 핵심 로직을 구현하고, 다음 단계로 팀스페이스
											세부 기능 범위를 확정합니다.
										</p>
									</div>
						...
						<div className="flex h-full flex-col rounded-[2rem] border border-emerald-300/40 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(255,255,255,0.98))] p-[1.625rem] shadow-panel">
							<div>
								...
								<p className="mt-[1.125rem] max-w-2xl text-base leading-8 text-emerald-950/80">
									서비스 기본 흐름과 운영 환경을 핵심 단위로 정리했습니다.
								</p>
							</div>
				...
				<PresentationSlide
					description="현재 구현은 매칭 요청 접수부터 상대방 확인, 수락·거절 처리까지 이어지는 핵심 로직에 집중되어 있습니다."
					id="in-progress"
					label="02"
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-4 text-base leading-7 text-amber-950/80">
										지금은 매칭 요청부터 수락·거절 처리까지 이어지는 핵심 로직을
										구현하는 단계입니다.
									</p>
								</div>

								<div className="grid gap-3">
									{["매칭 시스템 구현"].map((item, index) => (
										<div
