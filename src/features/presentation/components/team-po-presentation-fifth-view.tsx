...
const summaryCards = [
	{
		...
		value: "3 Items",
		description:
			"JWT 인증, 유저 관리 API, 그리고 연동된 프론트엔드 구현을 마무리했습니다.",
		icon: BadgeCheck,
		tone: "primary",
	},
	{
		title: "진행 중",
		value: "2 Items",
		description: "매칭 생성과 팀 관리 API를 구현하고 있습니다.",
		icon: Clock3,
		tone: "accent",
	},
	{
		title: "다음 목표",
		value: "Flow Integration",
		description: "인증-매칭-팀 기능을 하나의 사용자 흐름으로 연결합니다.",
		icon: Rocket,
		tone: "chart2",
	},
] as const;
...
const completedItems = [
	{
		title: "JWT기반 사용자 인증 및 토큰 재발급 API 구현",
		description:
			"로그인 이후에도 인증 상태를 안정적으로 유지할 수 있도록 JWT 인증과 토큰 재발급 흐름을 서버에 반영했습니다.",
		icon: KeyRound,
		keywords: ["JWT", "Refresh Token", "Auth API"],
	},
	{
		title: "유저 정보 수정 및 삭제 API 구현",
		description:
			"프로필 변경과 계정 삭제까지 처리할 수 있도록 유저 관리 API 범위를 확장했습니다.",
		icon: UsersRound,
		keywords: ["User Update", "User Delete", "Account API"],
	},
	{
		title: "구현된 유저 API에 맞게 프론트엔드 구현",
		description:
			"백엔드 응답 구조에 맞춰 사용자 정보 관련 화면을 연결해 실제 사용 흐름을 프론트엔드에서 확인할 수 있게 했습니다.",
		icon: Target,
		keywords: ["Frontend", "API Integration", "User Flow"],
	},
] as const;

const inProgressItems = [
	{
		title: "매칭 생성 및 시스템 구현",
		description:
			"핵심 서비스인 매칭 요청부터 결과 생성까지의 기본 흐름을 연결하고 있습니다.",
		icon: GitBranch,
		keywords: ["Matching", "Generate", "Core Logic"],
	},
	{
		title: "팀 생성 및 기본 관리 API 구현",
		description:
			"매칭 이후 팀이 실제로 생성되고 관리될 수 있도록 팀 도메인 API를 확장하고 있습니다.",
		icon: UsersRound,
		keywords: ["Team Create", "Management", "Domain API"],
	},
] as const;

const plannedItems = [
	{
		title: "인증, 매칭, 팀 기능 사용자 흐름 통합",
		description:
			"핵심 기능이 완성되는 대로 하나의 end-to-end 시나리오로 묶어 실제 사용 흐름을 검증합니다.",
		icon: Layers3,
		keywords: ["E2E Flow", "Integration", "User Journey"],
	},
	{
		title: "팀스페이스 기능 스펙 확정 후 구현 시작",
		description:
			"팀 운영 단계 기능을 구체화한 뒤 다음 구현 축을 팀스페이스로 전환합니다.",
		icon: FolderKanban,
		keywords: ["Team Space", "Spec", "Next Build"],
	},
] as const;
...
const pageLabels = new Map(
	tocSections.map((section, index) => [section.id, `(${index + 1}/${tocSections.length})`]),
);

export function TeamPoPresentationFifthView() {
	...
	return (
		...
		>
			...
			<main className="relative pb-16">
					...
					<Container className="presentation-slide-inner relative rounded-[2rem] border border-border/60 bg-white/72 px-7 py-9 shadow-panel backdrop-blur-sm md:px-10 md:py-11">
						<div className="flex h-full flex-col gap-10">
							<div className="flex flex-wrap items-center gap-4">
								...
								<Badge variant="neutral">2026. 4. 1.</Badge>
								...
								<div className="relative flex h-full w-full flex-col justify-between gap-10">
									<div className="space-y-4">
										...
										<p className="max-w-3xl text-lg leading-8 text-muted-foreground md:text-[1.3rem]">
											JWT 인증과 유저 관리 API를 마무리했고, 이번 주에는 매칭과 팀
											생성 흐름으로 구현 범위를 확장하고 있습니다.
										</p>
									</div>
				...
				<PresentationSlide
					description="이번 주에 마무리된 작업은 유저 관련 API 작업을 안정적으로 마무리해가는 데 집중되어 있습니다."
					id="completed"
					label="01"
					...
					<div className="grid gap-[1.375rem] xl:grid-cols-[22rem_minmax(0,1fr)]">
						<div className="flex h-full flex-col rounded-[2rem] border border-emerald-300/40 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(255,255,255,0.98))] p-[1.625rem] shadow-panel">
							<div>
								...
								<p className="mt-[1.125rem] max-w-2xl text-base leading-8 text-emerald-950/80">
									인증 체계와 유저 관리 API를 마무리하고 프론트엔드 연동까지
									이어붙였습니다.
								</p>
							</div>
							...
							<div className="mt-[1.375rem] rounded-[1.6rem] border border-emerald-200/70 bg-white px-5 py-[1.125rem]">
								<div className="space-y-3">
									{completedItems.map((item, index) => (
										<div
											className="grid items-center gap-3 rounded-[1.15rem] border border-emerald-100 bg-emerald-50 px-4 py-4 md:grid-cols-[3rem_minmax(0,1fr)]"
											key={item.title}
										>
						...
						<div className="grid auto-rows-fr gap-4">
							{completedItems.map((item) => {
								...
								return (
									<Surface
										className="border-emerald-200/70 bg-white/88 p-5"
										key={item.title}
										variant="glass"
									>
				...
				<PresentationSlide
					id="landing"
					label="02"
					pageLabel={getPageLabel("landing")}
					title="랜딩 페이지"
				>
					<Surface
						className="border-slate-200/70 bg-white/90 p-3"
						variant="glass"
					>
						<div className="flex h-[38rem] items-center justify-center rounded-[1.6rem] border border-border/60 bg-slate-50 p-3 xl:h-[42rem]">
							<img
								alt="Team-po 랜딩 페이지 시안"
				...
				<PresentationSlide
					description="현재 구현은 핵심 도메인인 매칭과 팀 기능을 실제로 동작시키는 방향에 집중되어 있습니다."
					id="in-progress"
					label="03"
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-4 text-base leading-7 text-amber-950/80">
										지금은 매칭 생성과 팀 생성 흐름을 실제로 동작시키는 단계입니다.
									</p>
								</div>
				...
				<PresentationSlide
					description="다음 단계는 이미 진행 중인 기능을 하나의 사용자 경험으로 연결하고, 그 뒤 팀스페이스 구현으로 진입하는 것입니다."
					id="planned"
					label="04"
						...
						<Surface
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-4 text-2xl font-display text-sky-950">2 Items</p>
									<p className="mt-2 text-base leading-6 text-sky-950/80">
										현재 작업 종료 후 사용자 흐름 통합 → 팀스페이스로 이동.
									</p>
								</div>
