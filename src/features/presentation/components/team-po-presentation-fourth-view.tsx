...
const summaryCards = [
	{
		title: "완료",
		value: "3 Items",
		description: "인프라, 인증 API, 이메일 중복 검사를 마무리했습니다.",
		icon: BadgeCheck,
		tone: "primary",
	},
	{
		title: "진행 중",
		value: "3 Items",
		description: "JWT 인증, 매칭 생성, 팀 관리 API를 구현하고 있습니다.",
		icon: Clock3,
		tone: "accent",
	},
	{
...
const completedItems = [
	{
		title: "서버 인프라 및 CI/CD 파이프라인 구축",
		description:
			"배포와 검증 루프를 먼저 고정해 이후 API 개발을 안정적으로 진행할 수 있는 기반을 마련했습니다.",
		icon: Server,
		keywords: ["Infra", "CI/CD", "Deploy Loop"],
	},
	{
		title: "회원가입 / 로그인 API 개발",
		description:
			"사용자 생성과 인증 진입점을 우선 구현해 서비스의 기본 사용자 흐름을 열었습니다.",
		icon: LogIn,
		keywords: ["Signup", "Login", "Auth Entry"],
	},
	{
		title: "회원가입 유효성 검사 API 개발",
		description:
			"이메일 중복 검사까지 반영해 회원가입 과정의 핵심 검증 단계를 보완했습니다.",
		icon: MailCheck,
		keywords: ["Validation", "Email Check", "Guardrail"],
	},
] as const;

const inProgressItems = [
	{
		title: "JWT 기반 사용자 인증 및 토큰 재발급 구현",
		description:
			"로그인 이후 세션을 안정적으로 유지하기 위한 인증 체계를 정리하고 있습니다.",
		icon: KeyRound,
		keywords: ["JWT", "Refresh Token", "Session"],
	},
	{
...
export function TeamPoPresentationFourthView() {
	...
	return (
		...
		>
			...
			<main className="relative pb-16">
					...
					<Container className="presentation-slide-inner relative rounded-[2rem] border border-border/60 bg-white/72 px-7 py-9 shadow-panel backdrop-blur-sm md:px-10 md:py-11">
						<div className="flex h-full flex-col gap-10">
							<div className="flex flex-wrap items-center gap-4">
								...
								<Badge variant="neutral">2026. 3. 30.</Badge>
								...
								<div className="relative flex h-full w-full flex-col justify-between gap-10">
									<div className="space-y-4">
										...
										<p className="max-w-3xl text-lg leading-8 text-muted-foreground md:text-[1.3rem]">
											인프라와 인증 기반을 고정한 뒤, 이번 주에는 매칭과 팀 생성
											흐름으로 구현 범위를 확장하고 있습니다.
										</p>
									</div>
									...
									<div className="flex flex-wrap items-end justify-between gap-5 rounded-[1.75rem] border border-primary/10 bg-white/70 px-6 py-5 shadow-soft">
										...
										<div className="rounded-[1.4rem] border border-border/60 bg-background/80 px-5 py-4">
											<p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">
												Checkpoint
											</p>
											<p className="mt-2 text-base leading-7 text-brand-ink">
												기반 공사에서 실제 사용자 흐름으로 넘어가는 주간
											</p>
										</div>
									</div>
								</div>
				...
				<PresentationSlide
					description="이번 주에 마무리된 작업은 서비스 실행 기반과 사용자 진입 경로를 고정하는 데 집중되어 있습니다."
					id="completed"
					label="01"
						...
						<div className="flex h-full flex-col rounded-[2rem] border border-emerald-300/40 bg-[linear-gradient(180deg,rgba(236,253,245,0.98),rgba(255,255,255,0.98))] p-[1.625rem] shadow-panel">
							<div>
								...
								<p className="mt-[1.125rem] max-w-2xl text-base leading-8 text-emerald-950/80">
									기반 작업과 사용자 진입 API를 이번 주 안에 닫았습니다.
								</p>
							</div>
				...
				<PresentationSlide
					description="현재 구현은 인증을 안정화하고, 핵심 도메인인 매칭과 팀 기능을 실제로 동작시키는 방향에 집중되어 있습니다."
					id="in-progress"
					label="02"
							...
							<div className="relative flex h-full flex-col justify-between gap-5">
								<div>
									...
									<p className="mt-4 text-base leading-7 text-amber-950/80">
										지금은 로그인 이후의 인증 상태와 매칭, 팀 생성 흐름을 실제로
										동작시키는 단계입니다.
									</p>
								</div>
