...
interface TeamPoPresentationViewProps {
	courseLabel?: string;
	dateLabel?: string;
	teamLabel?: string;
	introEyebrow?: string;
	...
	showDefaultOverviewSlides?: boolean;
	showPageNumbers?: boolean;
	tocSections?: PresentationSectionLink[];
}

const PresentationPaginationContext = createContext<Map<string, string> | null>(
	null,
);
...
export function TeamPoPresentationView({
	...
	showDefaultOverviewSlides = true,
		...
		<PresentationPaginationContext.Provider value={pageLabels}>
			...
			>
				...
				<main className="relative pb-16">
						...
						<Container className="presentation-slide-inner relative rounded-[2rem] border border-border/60 bg-white/72 px-7 py-9 shadow-panel backdrop-blur-sm md:px-10 md:py-11">
							<div className="flex h-full flex-col gap-10">
								<div className="flex flex-wrap items-center gap-4">
									...
									<Badge variant="neutral">{teamLabel}</Badge>
								</div>
											...
											<div className="flex h-full flex-col justify-between gap-10">
												<div className="space-y-4">
													...
													<h1 className="text-6xl font-display leading-none md:text-[5.8rem]">
														<span className="ds-title-gradient">
															{brandName}
														</span>
													</h1>
													{introDescription ? (
					...
					<PresentationSlide id="toc" label="INDEX" title="목차">
										...
										<div className="relative flex h-full flex-col justify-between gap-4">
											<div>
												...
												<p className="mt-4 max-w-lg text-base leading-7 text-muted-foreground">
													초보 개발자에게 필요한 것은 단순한 모집 게시판이
													아니라, 팀이 만들어지고 유지되며 끝까지 완주할 수 있는
													구조입니다.
												</p>
											</div>
					...
					{showDefaultPostSolutionSlides ? (
						<>
										...
										<div className="grid auto-rows-fr gap-5 sm:grid-cols-2">
											...
											<div className="rounded-[2rem] border border-primary/15 bg-gradient-to-br from-primary/10 via-white to-chart-2/10 p-7 shadow-panel">
												...
												<div className="mt-6 space-y-4 text-base leading-8 text-slate-800">
													<p>
														주제·역할·기술 스택·온도를 함께 반영해 팀원을 구성
													</p>
													<p>
														매칭 완료 시 이메일 알림과 수락/거절 플로우 제공
													</p>
							...
							<PresentationSlide
								...
								title="역할 분담"
							>
								<div className="grid auto-rows-fr grid-cols-4 gap-5">
									...
									<div className="flex flex-col justify-between gap-8">
										<div>
											...
											<h2 className="mt-5 text-6xl font-display leading-none text-brand-ink md:text-[6rem]">
												<span className="ds-title-gradient">{brandName}</span>
											</h2>
